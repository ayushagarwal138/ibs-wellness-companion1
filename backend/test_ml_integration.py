...
class MLIntegrationTester:
    ...
    async def authenticate_user(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_test_result("User Authentication", True, "Successfully authenticated")
                return True
            else:
                self.log_test_result("User Authentication", False, f"Login failed: {response.status_code}")
                return False
    ...
    async def test_model_info_endpoint(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_test_result("Model Info Endpoint", False, f"Status code: {response.status_code}")
                return False
    ...
    async def test_severity_prediction(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_test_result("Severity Prediction", False, f"Status: {response.status_code}, Error: {error_detail}")
                return False
    ...
    async def test_flareup_prediction(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error_detail = response.text
                self.log_test_result("Flareup Prediction", False, f"Status: {response.status_code}, Error: {error_detail}")
                return False
    ...
    async def test_recommendations_endpoint(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error_detail = response.text
                self.log_test_result("Recommendations Endpoint", False, f"Status: {response.status_code}, Error: {error_detail}")
                return False
    ...
    async def test_model_reload_endpoint(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error_detail = response.text
                self.log_test_result("Model Reload Endpoint", False, f"Status: {response.status_code}, Error: {error_detail}")
                return False
    ...
    async def test_edge_cases(self) -> bool:
        ...
        try:
            ...
            if response.status_code in [200, 422]:  # Accept both success and validation error
                success_count += 1
                self.log_test_result("Edge Case - Empty Symptoms", True, f"Handled gracefully (status: {response.status_code})")
            else:
                self.log_test_result("Edge Case - Empty Symptoms", False, f"Unexpected status: {response.status_code}")
        except Exception as e:
            self.log_test_result("Edge Case - Empty Symptoms", False, f"Error: {str(e)}")
        ...
        try:
            response = await self.client.post(
                f"{API_BASE}/ml/predict/flareup",
                json={"days_ahead": 100, "symptoms": SAMPLE_SYMPTOMS},  # Invalid days_ahead
                headers=self.get_auth_headers()
            )
            
            if response.status_code in [200, 422]:  # Accept both success and validation error
                success_count += 1
                self.log_test_result("Edge Case - Invalid Days Ahead", True, f"Handled gracefully (status: {response.status_code})")
            else:
                self.log_test_result("Edge Case - Invalid Days Ahead", False, f"Unexpected status: {response.status_code}")
        except Exception as e:
            self.log_test_result("Edge Case - Invalid Days Ahead", False, f"Error: {str(e)}")
        
        # Test without authentication
        total_tests += 1
        try:
            response = await self.client.get(f"{API_BASE}/ml/models/info")  # No auth headers
            
            if response.status_code in [401, 403]:  # Should require authentication
                success_count += 1
                self.log_test_result("Edge Case - No Authentication", True, f"Properly rejected (status: {response.status_code})")
            else:
                self.log_test_result("Edge Case - No Authentication", False, f"Should require auth (status: {response.status_code})")
        except Exception as e:
            self.log_test_result("Edge Case - No Authentication", False, f"Error: {str(e)}")
