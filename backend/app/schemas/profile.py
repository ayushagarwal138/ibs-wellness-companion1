...
class BasicInfoUpdate(BaseModel):
    ...
    height_cm: Optional[float] = Field(
        None, gt=0, le=300, description="Height in centimeters"
    )
    weight_kg: Optional[float] = Field(
        None, gt=0, le=1000, description="Weight in kilograms"
    )
...
class MedicalHistoryUpdate(BaseModel):
    ...
    known_triggers: Optional[List[str]] = Field(
        None, description="Known trigger foods/factors"
    )
    common_symptoms: Optional[List[str]] = Field(
        None, description="Common symptoms experienced"
    )
    ...
    other_conditions: Optional[List[str]] = Field(
        None, description="Other medical conditions"
    )
    medical_notes: Optional[str] = Field(
        None, max_length=2000, description="Additional medical notes"
    )
...
class DietaryPreferencesUpdate(BaseModel):
    ...
    meal_frequency: Optional[int] = Field(
        None, ge=1, le=10, description="Meals per day"
    )
    water_intake_goal: Optional[float] = Field(
        None, ge=0, le=10, description="Daily water intake goal in liters"
    )
    special_diets: Optional[List[str]] = Field(
        None, description="Special diets followed"
    )
    ...
    def validate_food_lists(cls, v):
        ...
        if v is not None:
            cleaned = list(
                set([item.strip().lower() for item in v if item and item.strip()])
            )
...
class LifestyleFactorsUpdate(BaseModel):
    ...
    exercise_frequency: Optional[ExerciseFrequencyEnum] = Field(
        None, description="Exercise frequency"
    )
    sleep_quality: Optional[int] = Field(
        None, ge=1, le=10, description="Sleep quality rating 1-10"
    )
    stress_level: Optional[int] = Field(
        None, ge=1, le=10, description="Stress level rating 1-10"
    )
    work_schedule: Optional[str] = Field(
        None, max_length=100, description="Work schedule description"
    )
    smoking_status: Optional[str] = Field(
        None, max_length=50, description="Smoking status"
    )
    alcohol_consumption: Optional[str] = Field(
        None, max_length=50, description="Alcohol consumption level"
    )
...
class GoalsPreferencesUpdate(BaseModel):
    ...
    preferred_treatments: Optional[List[str]] = Field(
        None, description="Preferred treatment approaches"
    )
    communication_preferences: Optional[List[str]] = Field(
...
class ProfileCompletionStatus(BaseModel):
    ...
    section_completion: Dict[str, float] = Field(
        ..., description="Completion percentage by section"
    )
