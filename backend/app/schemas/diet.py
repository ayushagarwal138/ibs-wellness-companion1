...
class FoodReactionBase(BaseModel):
    ...
    onset_time: Optional[int] = Field(
        None, ge=0, description="Time after eating when reaction started (minutes)"
    )
    duration_minutes: Optional[int] = Field(
        None, ge=0, description="Duration of reaction in minutes"
    )
    notes: Optional[str] = Field(None, max_length=1000, description="Additional notes")
    consumed_at: Optional[datetime] = Field(
        None, description="When the food was consumed"
    )
...
class DietLogBase(BaseModel):
    ...
    portion_size: Optional[str] = Field(
        None, max_length=100, description="Portion size description"
    )
    ...
    mood_before: Optional[int] = Field(
        None, ge=1, le=10, description="Mood rating 1-10"
    )
