...
class GoalBase(BaseModel):
    ...
    target_value: Optional[float] = Field(None, description="Target value for measurable goals")
...
class GoalProgressBase(BaseModel):
    ...
    recorded_at: Optional[datetime] = Field(default_factory=datetime.utcnow, description="When progress was recorded")
...
class BulkGoalProgressCreate(BaseModel):
    ...
    def validate_unique_goals(cls, v):
        ...
        if len(goal_ids) != len(set(goal_ids)):
            raise ValueError('Duplicate goal IDs are not allowed in bulk progress update')
        return v
