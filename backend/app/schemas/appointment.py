...
class AppointmentBase(BaseModel):
    ...
    title: str = Field(
        ..., min_length=1, max_length=200, description="Appointment title"
    )
    description: Optional[str] = Field(
        None, max_length=1000, description="Appointment description"
    )
    appointment_type: AppointmentTypeEnum = Field(
        ..., description="Type of appointment"
    )
    scheduled_date: date = Field(..., description="Date of appointment")
    scheduled_time: time = Field(..., description="Time of appointment")
    duration_minutes: int = Field(
        default=30, ge=15, le=480, description="Duration in minutes"
    )
    provider_name: Optional[str] = Field(
        None, max_length=200, description="Healthcare provider name"
    )
    provider_specialty: Optional[str] = Field(
        None, max_length=100, description="Provider specialty"
    )
    location: Optional[str] = Field(
        None, max_length=500, description="Appointment location"
    )
    is_virtual: bool = Field(
        default=False, description="Whether appointment is virtual"
    )
    virtual_link: Optional[str] = Field(None, description="Virtual meeting link")
    priority: AppointmentPriorityEnum = Field(
        default=AppointmentPriorityEnum.MEDIUM, description="Appointment priority"
    )
    notes: Optional[str] = Field(None, max_length=1000, description="Additional notes")
    preparation_instructions: Optional[str] = Field(
        None, max_length=1000, description="Preparation instructions"
    )
...
class AppointmentOutcomeBase(BaseModel):
    ...
    diagnosis: Optional[str] = Field(
        None, max_length=500, description="Diagnosis or findings"
    )
    treatment_plan: Optional[str] = Field(
        None, max_length=1000, description="Treatment plan"
    )
    medications_prescribed: Optional[List[str]] = Field(
...
class BulkAppointmentCreate(BaseModel):
    ...
    def validate_no_time_conflicts(cls, v):
        ...
        time_slots = []
        for apt in v:
            slot = (apt.scheduled_date, apt.scheduled_time)
