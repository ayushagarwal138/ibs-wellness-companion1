...
class DataOptimizationService:
    ...
    async def optimize_diet_logs(self, level: OptimizationLevel = OptimizationLevel.STANDARD) -> OptimizationMetrics:
        ...
        cutoff_date = datetime.utcnow() - timedelta(days=self.retention_policies['diet_logs'])
        
        # Count records to be processed
        count_query = select(func.count(DietLog.id)).where(DietLog.consumed_at < cutoff_date)
    ...
    async def optimize_indexes(self) -> OptimizationMetrics:
        ...
        tables_to_analyze = ['diet_logs', 'symptoms', 'ml_predictions', 'users', 'notifications']
    ...
    async def cleanup_old_data(self) -> OptimizationMetrics:
        ...
        audit_cutoff = datetime.utcnow() - timedelta(days=self.retention_policies['audit_logs'])
        audit_delete_query = text(
            "DELETE FROM audit_logs WHERE created_at < :cutoff_date"
        )
        result = await self.db.execute(audit_delete_query, {"cutoff_date": audit_cutoff})
        audit_deleted = result.rowcount
        total_records_deleted += audit_deleted
        logger.info(f"Deleted {audit_deleted} old audit logs")
        
        # Clean up old notifications
        notif_cutoff = datetime.utcnow() - timedelta(days=self.retention_policies['notifications'])
        notif_delete_query = text(
            "DELETE FROM notifications WHERE created_at < :cutoff_date"
        )
        result = await self.db.execute(notif_delete_query, {"cutoff_date": notif_cutoff})
        ...
        ml_cutoff = datetime.utcnow() - timedelta(days=self.retention_policies['ml_predictions'])
    ...
    async def run_full_optimization(self, level: OptimizationLevel = OptimizationLevel.STANDARD) -> List[OptimizationMetrics]:
        ...
        try:
            ...
            logger.info(f"Current storage analysis: {len(storage_stats)} tables analyzed")
