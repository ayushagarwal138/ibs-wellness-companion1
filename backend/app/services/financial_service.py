...
from app.models.financial import (
    ...
    MedicationCost, Invoice, PaymentStatusEnum, TransactionTypeEnum,
    SubscriptionStatusEnum, PaymentMethodTypeEnum
)
from app.models.medication import Medication
from app.schemas.financial import (
    PaymentMethodCreate, TransactionCreate, SubscriptionCreate,
    MedicationCostCreate, InvoiceCreate
)
...
class FinancialService:
    ...
    async def cancel_subscription(self, subscription_id: str, user_id: str) -> Subscription:
    ...
    async def get_spending_analytics(
        ...
        try:
            ...
            ).group_by(func.date_trunc('month', Transaction.created_at)).order_by('month')
            ...
            stmt = select(
                ...
                MedicationCost, Medication.id == MedicationCost.medication_id
            ).where(
                and_(
            ...
            ).group_by(Medication.name).order_by(desc('total_cost')).limit(10)
            
            result = await self.db.execute(stmt)
            top_medications = [
                {'medication': row.name, 'total_cost': row.total_cost}
                for row in result
            ]
