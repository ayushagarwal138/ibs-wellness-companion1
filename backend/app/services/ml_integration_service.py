...
class MLIntegrationService:
    ...
    def _load_models(self):
        ...
        try:
            ...
            flareup_model_path = (
                self.models_path / "checkpoints" / "flareup_predictor.pkl"
            )
            ...
            severity_model_path = (
                self.models_path / "checkpoints" / "severity_classifier.pkl"
            )
            ...
            recommendation_model_path = (
                self.models_path / "checkpoints" / "recommendation_engine.pkl"
            )
            if recommendation_model_path.exists():
                self.models["recommendation_engine"] = joblib.load(
                    recommendation_model_path
                )
    ...
    def predict_flareup_risk(self, user: User, days_ahead: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            if "feature_scaler" in self.scalers:
                try:
                    feature_array = self.scalers["feature_scaler"].transform(
                        feature_array
                    )
                except Exception:
            ...
            if "flareup_predictor" in self.models:
                try:
                    risk_proba = self.models["flareup_predictor"].predict_proba(
                        feature_array
                    )[0]
        ...
        try:
            ...
            if features and len(features) >= 5:
                ...
                assessment.confidence_score = min(
                    assessment.confidence_score + 0.1, 1.0
                )
    ...
    def generate_onboarding_predictions(
        ...
        try:
            ...
            predictions = {
                ...
                "lifestyle_insights": self._generate_lifestyle_insights(
                    onboarding_data
                ),
                "dietary_recommendations": self._generate_dietary_recommendations(
                    onboarding_data
                ),
                "management_strategy": self._suggest_management_strategy(
                    onboarding_data
                ),
    ...
    def _predict_risk_level(
        self, features: List[float], data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if risk_score < 30:
            risk_level = "Low"
            description = (
                "Your symptoms appear manageable with lifestyle modifications."
            )
        elif risk_score < 60:
            risk_level = "Moderate"
    ...
    def _analyze_triggers(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        stress_triggers = [
            t
            for t in triggers
            if t in ["work_stress", "emotional_stress", "lack_of_sleep"]
        ]
        lifestyle_triggers = [
            t
            for t in triggers
            if t in ["irregular_meals", "travel", "hormonal_changes"]
        ]

        analysis = {
            "primary_category": "food"
            if len(food_triggers) >= len(stress_triggers)
            else "stress",
        ...
        if len(food_triggers) > 2:
            analysis["insights"].append(
                "Consider an elimination diet to identify specific food sensitivities."
            )

        if len(stress_triggers) > 1:
            analysis["insights"].append(
                "Stress management techniques could significantly improve your symptoms."
            )

        if len(triggers) > 5:
            analysis["insights"].append(
                "Multiple triggers suggest a comprehensive management approach is needed."
            )
    ...
    def _generate_dietary_recommendations(
        ...
        triggers = data.get("knownTriggers", [])
        _dietary_restrictions = data.get("dietaryRestrictions", [])
        ibs_type = data.get("ibsType", "ibs-u")

        # FODMAP recommendation
        if any(
            trigger in triggers for trigger in ["dairy", "gluten", "high_fat_foods"]
        ):
    ...
    def _generate_personalized_tips(self, data: Dict[str, Any]) -> List[str]:
        ...
        if age > 50:
            tips.append(
                "Consider discussing hormone-related IBS changes with your healthcare provider"
            )
        elif age < 30:
            tips.append(
                "Establish consistent routines early to better manage symptoms long-term"
            )
        ...
        if gender == "female":
            tips.append(
                "Track symptoms in relation to your menstrual cycle for pattern recognition"
            )
        ...
        if ibs_type == "ibs-d":
            tips.append(
                "Keep anti-diarrheal medication handy when traveling or during stressful periods"
            )
        elif ibs_type == "ibs-c":
            tips.append(
                "Establish a regular bathroom routine, ideally at the same time each day"
            )

        # Stress-related tips
        if data.get("stressLevel", 5) > 6:
            tips.append(
                "Consider keeping a stress-symptom diary to identify your personal stress triggers"
            )
    ...
    def _extract_user_features(self, user: User) -> List[float]:
        ...
        try:
            ...
            features.append(
                len([s for s in recent_symptoms if s.severity == SeverityEnum.SEVERE])
            )
            features.append(
                len([s for s in recent_symptoms if s.severity == SeverityEnum.MODERATE])
            )
            ...
            severe_reactions = len(
                [r for r in food_reactions if r.severity == ReactionSeverityEnum.SEVERE]
            )
    ...
    def _calculate_fallback_risk(self, features: List[float]) -> float:
        ...
        risk_score = (
            min(symptom_count / 20, 1.0) * 0.3
            + min(severe_symptoms / 10, 1.0) * 0.4  # Symptom frequency
            + min(pain_level / 10, 1.0) * 0.2  # Severe symptoms
            + min(food_reactions / 15, 1.0) * 0.1  # Pain level  # Food reactions
        )
    ...
    def _predict_ml_severity(self, features: List[float]) -> IBSSeverity:
        ...
        try:
            if "severity_classifier" in self.models:
                feature_array = np.array(features).reshape(1, -1)
                prediction = self.models["severity_classifier"].predict(feature_array)[
                    0
                ]
    ...
    def _combine_severity_assessments(
        self, rule_based: IBSSeverity, ml_based: IBSSeverity
    ) -> IBSSeverity:
    ...
    def _build_user_profile(self, user: User, features: List[float]) -> Dict[str, Any]:
        ...
        return {
            ...
            "food_sensitivity": "high"
            if features and len(features) > 5 and features[5] > 10
            else "moderate",
        }
    ...
    def _apply_ml_personalization(
        self, base_recs: List[Dict], features: List[float], profile: Dict
    ) -> List[Recommendation]:
    ...
    def _apply_heuristic_personalization(
        self, base_recs: List[Dict], profile: Dict
    ) -> List[Recommendation]:
