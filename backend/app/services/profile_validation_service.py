...
from app.schemas.profile import (
    BasicInfoUpdate, MedicalHistoryUpdate, DietaryPreferencesUpdate,
    LifestyleFactorsUpdate, GoalsPreferencesUpdate, ComprehensiveProfileUpdate,
    ProfileValidationResponse, ProfileCompletionStatus
)
...
class ProfileValidationService:
    ...
    def __init__(self):
        self.required_fields = {
            'basic_info': ['first_name', 'last_name', 'email', 'date_of_birth', 'gender'],
        ...
        self.field_weights = {
            'basic_info': {
                ...
                'emergency_contact_name': 2, 'emergency_contact_phone': 2
            },
            'medical_history': {
                'ibs_type': 15, 'diagnosis_date': 10, 'severity_level': 8,
                'known_triggers': 6, 'common_symptoms': 6, 'symptom_patterns': 4,
                'current_medications': 5, 'allergies': 4, 'other_conditions': 3,
                'medical_notes': 2
            },
            'dietary_preferences': {
                'dietary_restrictions': 4, 'food_allergies': 5, 'preferred_cuisines': 2,
                'meal_frequency': 3, 'water_intake_goal': 2, 'special_diets': 3,
                'trigger_foods': 6, 'safe_foods': 4
            },
            'lifestyle_factors': {
                'exercise_frequency': 4, 'sleep_quality': 5, 'stress_level': 5,
                'work_schedule': 2, 'smoking_status': 3, 'alcohol_consumption': 3
            },
            'goals_preferences': {
                'primary_goals': 8, 'preferred_treatments': 4, 'communication_preferences': 2,
                'notification_preferences': 2, 'privacy_settings': 2
            }
        }
    ...
    async def validate_profile_update(
        ...
        try:
            ...
            if 'basic_info' in update_data or any(field in update_data for field in ['first_name', 'last_name', 'email', 'phone_number', 'date_of_birth', 'gender', 'height_cm', 'weight_kg']):
                basic_errors, basic_warnings = await self._validate_basic_info(update_data, current_user, db)
                if basic_errors:
            ...
            if 'medical_history' in update_data or any(field in update_data for field in ['ibs_type', 'diagnosis_date', 'medical_notes']):
                medical_errors, medical_warnings = await self._validate_medical_history(update_data, current_user)
                if medical_errors:
            ...
            if 'dietary_preferences' in update_data:
                dietary_errors, dietary_warnings = self._validate_dietary_preferences(update_data['dietary_preferences'])
                if dietary_errors:
            ...
            if 'lifestyle_factors' in update_data:
                lifestyle_errors, lifestyle_warnings = self._validate_lifestyle_factors(update_data['lifestyle_factors'])
                if lifestyle_errors:
    ...
    async def _validate_basic_info(
        ...
        if 'email' in update_data and update_data['email'] != current_user.email:
            ...
            existing_user = await user_service.get_user_by_email(db, update_data['email'])
        ...
        if height and weight:
            ...
            if bmi < 15 or bmi > 50:
                warnings.append('BMI appears to be outside normal range. Please verify height and weight.')
    ...
    def _validate_dietary_preferences(self, dietary_data: Dict[str, Any]) -> Tuple[List[str], List[str]]:
        """Validate dietary preferences."""
        errors = []
        warnings = []
        ...
        if 'water_intake_goal' in dietary_data:
            ...
            if water and (water < 0.5 or water > 8):
                warnings.append('Water intake goal seems unusual. Recommended range is 1.5-3 liters per day.')
        
        return errors, warnings
    
    def _validate_lifestyle_factors(self, lifestyle_data: Dict[str, Any]) -> Tuple[List[str], List[str]]:
        """Validate lifestyle factors."""
        errors = []
        warnings = []
    ...
    def _generate_suggestions(self, update_data: Dict[str, Any], current_user: User) -> List[str]:
        ...
        if not current_user.height_cm or not current_user.weight_kg:
            suggestions.append('Adding height and weight helps provide more accurate health insights')
        
        if not current_user.emergency_contact_name:
            suggestions.append('Consider adding emergency contact information for safety')
        
        # Medical history suggestions
        if not current_user.ibs_type:
            suggestions.append('Specifying your IBS type helps personalize recommendations')
        ...
        if not current_user.medical_notes:
            suggestions.append('Medical notes can help healthcare providers understand your condition better')
    ...
    def calculate_profile_completion(self, user: User) -> ProfileCompletionStatus:
        ...
        for section, fields in self.field_weights.items():
            ...
            section_percentage = (section_score / section_weight) * 100 if section_weight > 0 else 0
            section_scores[section] = round(section_percentage, 1)
            
            total_weight += section_weight
            total_score += section_score
        
        overall_completion = (total_score / total_weight) * 100 if total_weight > 0 else 0
        
        # Generate recommendations
        recommendations = []
        if overall_completion < 50:
            recommendations.append('Complete basic information to unlock personalized features')
        ...
        if overall_completion < 80:
            recommendations.append('Complete more sections for comprehensive health insights')
    ...
    def transform_frontend_to_backend(self, update_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def transform_backend_to_frontend(self, user_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'gender' in user_data and user_data['gender']:
            ...
            transformed['gender'] = gender_mapping.get(user_data['gender'], user_data['gender'])
        ...
        if 'ibs_type' in user_data and user_data['ibs_type']:
            ...
            transformed['ibs_type'] = ibs_mapping.get(user_data['ibs_type'], user_data['ibs_type'])
