...
class ConversationMemoryService:
    ...
    def add_conversation_turn(self, user_id: str, user_message: str, bot_response: Dict[str, Any]) -> None:
    ...
    def get_conversation_context(self, user_id: str, turns_back: int = 5) -> Dict[str, Any]:
        ...
        recent_turns = self.conversations[user_id][-turns_back:] if self.conversations[user_id] else []
        
        return {
            "recent_turns": [asdict(turn) for turn in recent_turns],
            "current_state": self.conversation_states.get(user_id, ConversationState.GREETING),
    ...
    def get_contextual_insights(self, user_id: str) -> Dict[str, Any]:
        ...
        engagement_level = "high" if len(turns) > 20 else "medium" if len(turns) > 5 else "new_user"
        
        # Analyze response preferences
        avg_response_length = sum(len(turn.bot_response) for turn in recent_turns) / len(recent_turns) if recent_turns else 0
        preferred_style = "detailed" if avg_response_length > 200 else "concise"
        
        # Extract common concerns
        concerns = []
        for turn in recent_turns:
            if any(word in turn.user_message.lower() for word in ["pain", "severe", "worried", "concerned"]):
                concerns.append("pain_management")
            if any(word in turn.user_message.lower() for word in ["diet", "food", "eat"]):
    ...
    def should_follow_up(self, user_id: str) -> Dict[str, Any]:
        ...
        last_turn = self.conversations[user_id][-1] if self.conversations[user_id] else None
        if not last_turn:
            return {"should_follow_up": False}
    ...
    def _update_conversation_state(self, user_id: str, bot_response: Dict[str, Any]) -> None:
    ...
    def _generate_conversation_summary(self, user_id: str) -> Dict[str, Any]:
        ...
        symptom_discussions = sum(1 for turn in turns if "symptom" in turn.intent.lower())
        assessments = sum(1 for turn in turns if "assessment" in turn.intent.lower())
        recommendations_requested = sum(1 for turn in turns if "recommendation" in turn.intent.lower())
        
        # Get most recent topics
        recent_topics = []
        for turn in turns[-5:]:
            if turn.entities:
                recent_topics.extend([entity.get("value", "") for entity in turn.entities])
    ...
    def clear_old_conversations(self, days_old: int = 30) -> None:
        ...
        for user_id in list(self.conversations.keys()):
            if self.conversations[user_id]:
                ...
                self.conversations[user_id] = [
                    turn for turn in self.conversations[user_id]
