...
class ChatService:
    ...
    def __init__(self, db: AsyncSession):
        ...
        self.greeting_messages = [
            (
                "Hello! I'm your IBS wellness assistant. I'm here to help you "
                "manage your symptoms and improve your quality of life."
            ),
            (
                "Hi there! Ready to take control of your IBS journey? I can help "
                "with symptom tracking, diet advice, and personalized recommendations."
            ),
            (
                "Welcome! I'm here to support you with your IBS management. "
                "Whether you need symptom analysis or lifestyle tips, I've got you covered."
            ),
            (
                "Hello! Let's work together to understand your IBS patterns "
                "and find what works best for you."
            ),
            (
                "Hi! I'm your personal IBS companion. I can help you track symptoms, "
                "identify triggers, and provide tailored advice."
            ),
        ]
    ...
    async def create_session(
        self, user: User, title: Optional[str] = None
    ) -> ChatSessionResponse:
        """Create a new chat session for the user."""
        session_data = ChatSessionCreate(
            title=title
            or f"Chat Session - {datetime.utcnow().strftime('%Y-%m-%d %H:%M')}",
    ...
    async def send_message(
        self, user: User, session_id: str, message: str, include_assessment: bool = True
    ) -> ChatbotResponse:
        ...
        # Store user message
        _user_message = await self._add_user_message(session.id, user.id, message)
        ...
        response_data = await self._generate_response(
            user, message, context, include_assessment
        )
    ...
    async def get_session_history(
        self, user: User, session_id: str, limit: int = 50
    ...
    async def _build_conversation_context(
        ...
        recent_medications = await self._get_recent_user_data(
            user.id, "medications", 30
        )
    ...
    async def _generate_response(
        ...
        enhanced_context = ConversationContext(
            ...
            if hasattr(conversation_context["current_state"], "value")
            else str(conversation_context["current_state"]),
            pending_followups=conversation_context["pending_followups"],
        )

        # Analyze message with NLP service using enhanced context
        nlp_result = self.nlp.analyze_message(
            message, enhanced_context.conversation_history
        )
        ...
        response_data = {
            ...
            "metadata": {
                ...
                "entities": [
                    {"type": e.type, "value": e.value} for e in nlp_result.entities
                ],
            },
            "context_used": [],
            "confidence": nlp_result.confidence,
        ...
        if (
            follow_up_check["should_follow_up"]
            and not nlp_result.intent == IntentType.GREETING
        ):
            # Handle follow-up scenarios
            response_data.update(
                await self._handle_follow_up(
                    user, enhanced_context, nlp_result, follow_up_check
                )
            )
        else:
            ...
            if nlp_result.intent == IntentType.GREETING:
                response_data.update(
                    self._handle_greeting(user, enhanced_context, nlp_result)
                )

            elif nlp_result.intent in [
                IntentType.SYMPTOM_REPORT,
                IntentType.SYMPTOM_INQUIRY,
            ]:
                response_data.update(
                    await self._handle_symptom_discussion(
                        user, message, enhanced_context, include_assessment, nlp_result
                    )
                )

            elif nlp_result.intent in [IntentType.FOOD_REPORT, IntentType.FOOD_INQUIRY]:
                response_data.update(
                    await self._handle_food_discussion(
                        user, message, enhanced_context, nlp_result
                    )
                )
                ...
                response_data.update(
                    await self._handle_medication_discussion(
                        user, message, enhanced_context, nlp_result
                    )
                )
                ...
                response_data.update(
                    await self._handle_recommendation_request(
                        user, enhanced_context, include_assessment, nlp_result
                    )
                )

            elif nlp_result.intent == IntentType.ASSESSMENT_REQUEST:
                response_data.update(
                    await self._handle_assessment_request(
                        user, enhanced_context, nlp_result
                    )
                )
        ...
        response_data = self._enhance_response_with_context(
            response_data, contextual_insights
        )
        ...
        response_data.update(
            {
                "conversation_context": {
                    "state": enhanced_context.current_state.value
                    if hasattr(enhanced_context.current_state, "value")
                    else str(enhanced_context.current_state),
    ...
    def _handle_greeting(
        self, user: User, context: ConversationContext, nlp_result
    ) -> Dict[str, Any]:
        ...
        sentiment = (
            nlp_result.sentiment.value
            if hasattr(nlp_result.sentiment, "value")
            else str(nlp_result.sentiment)
        )
    ...
    async def _handle_symptom_discussion(
        ...
        if "pain" in " ".join(symptoms).lower():
            ...
            if any(
                word in message.lower() for word in ["severe", "intense", "unbearable"]
            ):
                response_parts.append(
                    "Given the intensity you're describing, it's important to track this carefully. "
                )

        if "bloating" in " ".join(symptoms).lower():
            response_parts.append(
                "Bloating is a common IBS symptom that can often be managed through dietary adjustments. "
            )
        ...
        if context.recent_symptoms:
            ...
            response_parts.append(
                f"I can see you've logged {symptom_count} symptom entries in the past month. "
            )

            # Identify patterns
            if symptom_count >= 5:
                response_parts.append(
                    "Based on your recent logs, I can help identify patterns and triggers. "
                )
        ...
        if include_assessment:
            try:
                ...
                response_parts.append(
                    f"Based on your recent data, your current IBS severity appears to be {assessment.severity.value}. "
                )
                ...
                recommendations = (
                    self.recommendation_service.generate_enhanced_recommendations(
                        user, assessment
                    )
                )
                if recommendations:
                    response_parts.append(
                        "I have some ML-enhanced personalized recommendations that might help. "
                    )

            except Exception as _e:
                response_parts.append(
                    "I'm having trouble accessing your recent data for assessment. Please make sure you've been logging your symptoms regularly. "
                )

        # Add recommendations
        response_parts.append(
            "Would you like me to suggest some management strategies or provide dietary recommendations?"
        )
    ...
    async def _handle_food_discussion(
        self, user: User, message: str, context: ConversationContext, nlp_result
    ) -> Dict[str, Any]:
        ...
        response_parts.append(
            "Diet plays a crucial role in managing IBS symptoms. Let me share some insights based on your data."
        )

        if context.recent_foods:
            reaction_count = len(context.recent_foods)
            response_parts.append(
                f"You've logged {reaction_count} food reactions recently."
            )
            ...
            if trigger_foods:
                ...
                response_parts.append(
                    f"Foods that seem to trigger reactions for you include: {triggers_text}."
                )
                response_parts.append(
                    "Consider eliminating these foods temporarily to see if symptoms improve."
                )
        else:
            response_parts.append(
                "I don't see much food tracking data yet. Keeping a food diary can help identify triggers."
            )

        # General diet advice
        response_parts.append(
            "The low-FODMAP diet is often effective for IBS management. Would you like specific guidance on this approach?"
        )
    ...
    async def _handle_medication_discussion(
        self, user: User, message: str, context: ConversationContext, nlp_result
    ) -> Dict[str, Any]:
        ...
        if context.recent_medications:
            med_count = len(
                set(med.get("medication_name") for med in context.recent_medications)
            )
            response_parts.append(
                f"I see you're currently tracking {med_count} different medications."
            )
            response_parts.append(
                "Medication adherence is important for managing IBS effectively."
            )
        else:
            response_parts.append("I don't see any medication tracking data yet.")
            response_parts.append(
                "If you're taking IBS medications, tracking their effectiveness can be very helpful."
            )

        response_parts.append(
            "Remember, I can't provide medical advice, but I can help you track patterns and prepare questions for your healthcare provider."
        )
        ...
        try:
            ...
            recommendations = (
                self.recommendation_service.generate_enhanced_recommendations(
                    user,
                    assessment,
                    context.user_context if hasattr(context, "user_context") else None,
                )
            )

            # Get flare-up risk prediction from the enhanced service
            user_features = self.recommendation_service._extract_user_features(
                user, assessment
            )
            flareup_risk = self.recommendation_service.predict_symptom_risk(
                user_features
            )

            response_parts.append(
                f"Based on your current IBS severity level ({assessment.severity.value}), here are my top ML-enhanced recommendations:"
            )

            # Add flare-up risk information
            if flareup_risk["risk_level"] != "Low":
                response_parts.append(
                    f"⚠️ Symptom risk prediction: {flareup_risk['risk_level']} ({flareup_risk['confidence']:.0%} confidence)"
                )
                if flareup_risk.get("key_factors"):
                    response_parts.append(
                        f"Key risk factors: {', '.join(flareup_risk['key_factors'][:2])}"
                    )
            ...
            if len(recommendations) > 3:
                response_parts.append(
                    f"I have {len(recommendations) - 3} additional personalized recommendations available."
                )

        except Exception as e:
            self.logger.error(f"Error generating enhanced recommendations: {e}")
            response_parts.append(
                "I need more data to provide personalized recommendations."
            )
            response_parts.append(
                "Please make sure you're regularly logging your symptoms, food reactions, and medications."
            )
    ...
    async def _handle_progress_inquiry(
        ...
        try:
            ...
            if nlp_result.sentiment.value == "positive":
                response_parts.append(
                    "I'm glad you're interested in tracking your progress! "
                )
            elif nlp_result.sentiment.value == "negative":
                response_parts.append(
                    "I understand you might be concerned about your progress. Let me share what I've observed. "
                )
            else:
                response_parts.append(
                    "Let me give you an overview of your IBS management progress. "
                )
            ...
            if progress_data.get("symptom_trend"):
                ...
                if trend == "improving":
                    response_parts.append(
                        "Great news! Your symptoms have been trending in a positive direction over the past month. "
                    )
                elif trend == "worsening":
                    response_parts.append(
                        "I've noticed your symptoms have been more challenging lately. "
                    )
                else:
                    response_parts.append(
                        "Your symptoms have been relatively stable recently. "
                    )
            ...
            if progress_data.get("severity_changes"):
                ...
                response_parts.append(
                    f"Your average IBS severity has changed from {changes['previous']} to {changes['current']} over the past month. "
                )
            ...
            if progress_data.get("identified_triggers"):
                ...
                response_parts.append(
                    f"I've identified potential triggers including: {', '.join(triggers)}. "
                )
            ...
            if progress_data.get("successful_strategies"):
                ...
                response_parts.append(
                    f"Strategies that seem to be working well for you include: {', '.join(strategies)}. "
                )

            # Recommendations based on progress
            if progress_data.get("symptom_trend") == "improving":
                response_parts.append(
                    "Keep up the great work! Let's continue with your current approach and maybe explore some additional wellness strategies. "
                )
            elif progress_data.get("symptom_trend") == "worsening":
                response_parts.append(
                    "Let's work together to identify what might be causing this increase and adjust your management plan. "
                )
            else:
                response_parts.append(
                    "Your stable progress is good - let's see if we can optimize your approach for even better results. "
                )
                ...
                "quick_actions": [
                    {
                        ...
                        "action": "progress_recommendations",
                    },
    ...
    async def _handle_assessment_request(
        ...
        try:
            ...
            if flareup_risk["risk_level"] != "low":
                response_parts.append(
                    f"🔮 ML Prediction: {flareup_risk['risk_level']} risk of flare-up in next 7 days ({flareup_risk['risk_score']:.0%})"
                )
            ...
            response_parts.append(
                "Would you like personalized recommendations based on this assessment?"
            )
    ...
    async def _handle_general_question(
        ...
        _entities = [entity.value.lower() for entity in nlp_result.entities]

        # IBS-related knowledge base responses
        if any(
            keyword in question_keywords
            for keyword in ["what is ibs", "irritable bowel", "ibs symptoms"]
        ):
            return {
            ...
            keyword in question_keywords for keyword in ["diet", "food", "eat", "avoid"]
        ):
            return {
            ...
            keyword in question_keywords
            for keyword in ["stress", "anxiety", "mental health"]
        ):
            return {
            ...
            keyword in question_keywords
            for keyword in ["exercise", "physical activity", "workout"]
        ):
            return {
        ...
        # If no specific knowledge base match, provide intelligent general response
            ...
            if nlp_result.sentiment.value == "negative":
                ...
                base_response = (
                    "I'm glad you're taking an active interest in your health! "
                )
            else:
    ...
    async def _add_assistant_message(
        self, session_id: str, user_id: str, content: str, metadata: Dict = None
    ) -> ChatMessage:
    ...
    async def _get_recent_user_data(
        self, user_id: str, data_type: str, days: int
    ) -> List[Dict[str, Any]]:
    ...
    async def _get_user_progress(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "severity_changes": {"previous": "moderate", "current": "mild"},
    ...
    def _enhance_response_with_context(
        self, response_data: Dict[str, Any], contextual_insights: Dict
    ) -> Dict[str, Any]:
        """Enhance response with contextual insights and personalization."""

        # Add personalized elements based on user patterns
        if (
            contextual_insights.get("frequent_symptoms")
            and contextual_insights["frequent_symptoms"]
        ):
            response_data[
                "personalized_note"
            ] = f"Based on your history, I notice you often experience {', '.join(contextual_insights['frequent_symptoms'][:2])}. "
        ...
        if contextual_insights.get("suggested_actions"):
            ...
            response_data["quick_actions"].extend(
                contextual_insights["suggested_actions"]
            )

        # Add progress indicators if available
        if contextual_insights.get("progress_indicators"):
            response_data["progress_context"] = contextual_insights[
                "progress_indicators"
            ]
