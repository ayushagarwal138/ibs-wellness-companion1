...
class IBSDetectionService:
    ...
    async def assess_ibs_severity(self, user: User, days: int = 30) -> IBSAssessment:
        ...
        food_reactions_data = await self._get_food_reactions_data(
        ...
        frequency_score = self._calculate_frequency_score(
            symptoms_data, food_reactions_data
        )
    ...
    async def _get_food_reactions_data(
        self, user_id: str, start_date: datetime, end_date: datetime
    ) -> List[Dict]:
    ...
    async def _get_medication_data(
        self, user_id: str, start_date: datetime, end_date: datetime
    ) -> List[Dict]:
    ...
    def _calculate_symptoms_score(self, symptoms_data: List[Dict]) -> float:
        ...
        for symptom in symptoms_data:
            ...
            if symptom.get("bristol_scale"):
                ...
                if bristol in [
                    BristolStoolTypeEnum.TYPE_1,
                    BristolStoolTypeEnum.TYPE_2,
                ]:
                    bristol_score = 3  # Constipation
                elif bristol in [
                    BristolStoolTypeEnum.TYPE_6,
                    BristolStoolTypeEnum.TYPE_7,
                ]:
            ...
            symptom_total = (
                severity_score + pain_score + bloating_score + bristol_score
            ) / 4
    ...
    def _calculate_frequency_score(
        self, symptoms_data: List[Dict], food_reactions_data: List[Dict]
    ) -> float:
        """Calculate frequency score based on symptom and reaction frequency (0-10)."""
        days_with_symptoms = len(
            set(symptom["logged_at"].date() for symptom in symptoms_data)
        )

        days_with_reactions = len(
            set(reaction["consumed_at"].date() for reaction in food_reactions_data)
        )
    ...
    def _calculate_impact_score(
        self, symptoms_data: List[Dict], medication_data: List[Dict]
    ) -> float:
        ...
        if medication_data:
            ...
            unique_medications = len(
                set(med["medication_name"] for med in medication_data)
            )
        ...
        for symptom in symptoms_data:
            if symptom.get("notes"):
                ...
                keyword_count = sum(
                    1 for keyword in impact_keywords if keyword in notes_lower
                )
    ...
    def _identify_key_factors(
        ...
        if symptoms_data:
            avg_pain = sum(s.get("pain_level", 0) for s in symptoms_data) / len(
                symptoms_data
            )
            avg_bloating = sum(s.get("bloating_level", 0) for s in symptoms_data) / len(
                symptoms_data
            )
        ...
        if food_reactions_data:
            severe_reactions = [
                r
                for r in food_reactions_data
                if r["severity"]
                in [ReactionSeverityEnum.SEVERE, ReactionSeverityEnum.VERY_SEVERE]
            ]
            ...
            frequent_triggers = [
                food for food, count in food_counts.items() if count >= 2
            ]
            if frequent_triggers:
                factors.append(
                    f"Trigger foods identified: {', '.join(frequent_triggers[:3])}"
                )
        ...
        if medication_data:
            ...
            poor_adherence = [
                m for m in medication_data if m["adherence"] == AdherenceEnum.MISSED
            ]
    ...
    def get_severity_trend(self, user: User, periods: int = 3) -> List[Dict[str, Any]]:
        ...
        for i in range(periods):
            ...
            food_reactions_data = self._get_food_reactions_data(
                user.id, start_date, end_date
            )
