...
from ..models.symptom import SymptomLog, SeverityEnum
from ..models.diet import DietLog, FoodReaction
...
class IBSAssessmentService:
    ...
    async def conduct_quick_assessment(
        ...
        try:
            ...
            risk_assessment = await self._calculate_quick_risk_assessment(
                quick_data
            )
    ...
    async def _gather_user_data(
        ...
        if include_recent_data:
            ...
            recent_reactions = db.query(FoodReaction).filter(
                FoodReaction.user_id == user.id,
                FoodReaction.reaction_date >= datetime.utcnow() - timedelta(days=30)
            ).all()
            
            data.update({
                'recent_symptoms': [self._serialize_symptom(s) for s in recent_symptoms],
                'recent_diet': [self._serialize_diet_log(d) for d in recent_diet],
                'recent_reactions': [self._serialize_reaction(r) for r in recent_reactions],
            })
    ...
    async def _calculate_risk_assessment(
        ...
        if 'recent_symptoms' in user_data:
            symptom_score = self._analyze_symptom_severity(
                user_data['recent_symptoms']
            )
        ...
        if 'recent_reactions' in user_data:
            reaction_score = self._analyze_food_reactions(
                user_data['recent_reactions']
            )
    ...
    def _analyze_food_reactions(self, reactions: List[Dict[str, Any]]) -> float:
        ...
        severe_reactions = sum(1 for r in reactions 
    ...
    async def _classify_severity(self, user_data: Dict[str, Any]) -> str:
        ...
        avg_pain = sum(s.get('severity_level', 0) for s in symptoms 
                      if s.get('symptom_name') == 'abdominal_pain') / max(1, len(symptoms))
        
        symptom_frequency = len(symptoms) / 30  # Daily frequency
        
        if (avg_pain >= self.severity_criteria['severe']['pain_threshold'] and 
            symptom_frequency >= self.severity_criteria['severe']['frequency_threshold']):
            return 'severe'
        elif (avg_pain >= self.severity_criteria['moderate']['pain_threshold'] and 
              symptom_frequency >= self.severity_criteria['moderate']['frequency_threshold']):
            return 'moderate'
        else:
            return 'mild'
