...
class NutritionCalculator:
    ...
    async def get_food_nutrition_per_100g(self, food_name: str) -> Optional[NutritionData]:
    ...
    def calculate_macronutrient_breakdown(self, nutrition: NutritionData) -> MacronutrientBreakdown:
        ...
        fiber_percent = round((nutrition.fiber_g / nutrition.carbs_g) * 100, 1) if nutrition.carbs_g > 0 else 0
    ...
    async def calculate_daily_nutrition_summary(
        ...
        macronutrient_breakdown = self.calculate_macronutrient_breakdown(total_nutrition)
        
        # Calculate nutrition quality score (simplified)
        nutrition_quality_score = self._calculate_nutrition_quality_score(total_nutrition)
    ...
    async def get_nutrition_trends(
        ...
        while current_date < end_date:
            summary = await self.calculate_daily_nutrition_summary(user_id, current_date)
        ...
        avg_calories = sum(s['summary']['total_nutrition']['calories'] for s in daily_summaries) / total_days
        avg_protein = sum(s['summary']['total_nutrition']['protein_g'] for s in daily_summaries) / total_days
        avg_carbs = sum(s['summary']['total_nutrition']['carbs_g'] for s in daily_summaries) / total_days
        avg_fat = sum(s['summary']['total_nutrition']['fat_g'] for s in daily_summaries) / total_days
        avg_fiber = sum(s['summary']['total_nutrition']['fiber_g'] for s in daily_summaries) / total_days
    ...
    def _generate_nutrition_recommendations(
        ...
        if avg_protein < 50:
            recommendations.append("Consider adding more protein sources like lean meats, legumes, or dairy")
        
        if avg_calories < 1200:
            recommendations.append("Your calorie intake may be too low - consider consulting a nutritionist")
        
        protein_percent = (avg_protein * 4 / avg_calories) * 100 if avg_calories > 0 else 0
        if protein_percent < 15:
            recommendations.append("Aim for 15-35% of calories from protein for optimal health")
        
        fat_percent = (avg_fat * 9 / avg_calories) * 100 if avg_calories > 0 else 0
        if fat_percent > 35:
            recommendations.append("Consider reducing fat intake and focus on healthy fats")
        
        if not recommendations:
            recommendations.append("Your nutrition profile looks balanced - keep up the good work!")
