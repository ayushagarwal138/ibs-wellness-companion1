...
class EnhancedRecommendationService(RecommendationService):
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        for model_name in ["severity_model", "flareup_model", "recommendation_model"]:
            ...
            has_model = (
                model_name.replace("_model", "") in self.ml_models
                or len(self.ml_models) > 0
            )
            models_loaded[model_name] = has_model
            model_versions[model_name] = (
                "enhanced_v1.0.0" if has_model else "fallback_v1.0.0"
            )
    ...
    async def generate_enhanced_recommendations(
        self, user_id: int, ml_predictions: Dict[str, Any], db: AsyncSession
    ) -> Dict[str, Any]:
    ...
    async def _get_nutrition_optimization_recommendations(
        ...
        try:
            ...
            nutrition_guidelines = (
                await self.dynamic_data_service.get_personalized_nutrition_guidelines(
                    user_id
                )
            )
    ...
    async def _extract_user_features(
        ...
        try:
            ...
            counts_result = counts_query.first()
