...
class ResponseTemplateService:
    ...
    def __init__(self):
        self.greeting_templates = {
            ResponseTone.SUPPORTIVE: [
                ...
                (
                    "Hi there! I'm your wellness companion, ready to assist with any "
                    "IBS-related questions or concerns."
                ),
                (
                    "Welcome back! I'm here to provide personalized support for your "
                    "digestive health needs."
                ),
                (
                    "Hello! Let's work together to manage your IBS symptoms effectively. "
                    "What's on your mind?"
                ),
                (
                    "Hi! I'm here to offer guidance and support for your wellness journey. "
                    "How are you feeling today?"
                ),
            ],
            ResponseTone.ENCOURAGING: [
                (
                    "Hello! You're taking great steps by seeking support for your IBS "
                    "management. How can I help?"
                ),
                (
                    "Hi there! Every conversation is a step forward in your wellness journey. "
                    "What would you like to discuss?"
                ),
                (
                    "Welcome! You're doing amazing by being proactive about your health. "
                    "How can I assist you today?"
                ),
                (
                    "Hello! Your commitment to managing your IBS is inspiring. "
                    "What can I help you with?"
                ),
                (
                    "Hi! You're on the right path to better digestive health. "
                    "How can I support you today?"
                ),
            ],
            ResponseTone.ENTHUSIASTIC: [
                (
                    "Hello! I'm excited to help you on your IBS wellness journey! "
                    "What can we tackle together today?"
                ),
                (
                    "Hi there! Ready to make some positive changes for your digestive health? "
                    "Let's get started!"
                ),
                (
                    "Welcome! I'm thrilled to be your wellness companion. "
                    "What exciting progress can we make today?"
                ),
                (
                    "Hello! Let's turn your IBS challenges into victories! "
                    "How can I help you succeed today?"
                ),
                (
                    "Hi! Your wellness journey is full of possibilities. "
    ...
    def get_greeting_message(
        self, tone: ResponseTone = ResponseTone.SUPPORTIVE, user_name: Optional[str] = None
    ) -> str:
        """Get a personalized greeting message."""
        templates = self.greeting_templates.get(
            tone, self.greeting_templates[ResponseTone.SUPPORTIVE]
        )
        message = secrets.choice(templates)
    ...
    def get_symptom_acknowledgment(
        self, symptom: str, tone: ResponseTone = ResponseTone.EMPATHETIC
    ) -> str:
        """Get an empathetic symptom acknowledgment message."""
        templates = self.symptom_acknowledgment_templates.get(
            tone, self.symptom_acknowledgment_templates[ResponseTone.EMPATHETIC]
        )
        message = secrets.choice(templates)
        return message.format(symptom=symptom)

    def get_progress_celebration(
        self, tone: ResponseTone = ResponseTone.ENCOURAGING
    ) -> str:
        """Get a progress celebration message."""
        templates = self.progress_celebration_templates.get(
            tone, self.progress_celebration_templates[ResponseTone.ENCOURAGING]
        )
        return secrets.choice(templates)

    def get_recommendation_intro(
        self, recommendation_type: str, tone: ResponseTone = ResponseTone.SUPPORTIVE
    ) -> str:
        """Get an introduction for recommendations."""
        category_templates = self.recommendation_templates.get(
            recommendation_type, self.recommendation_templates["lifestyle"]
        )
        templates = category_templates.get(
            tone, category_templates[ResponseTone.SUPPORTIVE]
        )
        return secrets.choice(templates)
    ...
    def get_follow_up_message(self, follow_up_type: str, **kwargs) -> str:
        ...
        templates = self.follow_up_templates.get(
            follow_up_type, self.follow_up_templates["symptom_check"]
        )
        message = secrets.choice(templates)
        return message.format(**kwargs)

    def get_contextual_quick_actions(
        self, context_type: str, additional_actions: Optional[List[Dict]] = None
    ) -> List[Dict]:
        """Get contextual quick action buttons."""
        base_actions = self.quick_action_templates.get(
            context_type, self.quick_action_templates["general_support"]
        )
    ...
    def generate_dynamic_response(
        ...
        tone: ResponseTone = ResponseTone.SUPPORTIVE,
    ) -> Dict[str, Any]:
        ...
        if intent == "greeting":
            response["message"] = self.get_greeting_message(
                tone, context.get("user_name")
            )
            response["quick_actions"] = self.get_contextual_quick_actions(
                "general_support"
            )

        elif intent == "symptom_discussion":
            symptom = context.get("symptom", "symptoms")
            response["message"] = self.get_symptom_acknowledgment(
                symptom, ResponseTone.EMPATHETIC
            )
            response["quick_actions"] = self.get_contextual_quick_actions(
                "symptom_related"
            )
            ...
            if context.get("has_progress"):
                response["message"] = self.get_progress_celebration(
                    ResponseTone.ENCOURAGING
                )
            response["quick_actions"] = self.get_contextual_quick_actions(
                "assessment_related"
