"""
Dynamic Data Service for managing FODMAP data, nutrition guidelines, and recommendations.
This service replaces hardcoded values with database-driven dynamic content.
"""
...
from ..models.diet import Food, FoodCategoryEnum, FODMAPLevelEnum, DietLog
from ..models.user import User
from ..models.symptom import SymptomLog
from ..core.dynamic_config import get_config, RiskLevel, RecommendationPriority
...
class DynamicDataService:
    ...
    def get_fodmap_foods(self, fodmap_level: FODMAPLevelEnum = None) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def _get_food_alternatives(self) -> Dict[str, List[str]]:
        ...
        try:
            ...
            high_fodmap_foods = self.db.query(Food).filter(
                Food.fodmap_level == FODMAPLevelEnum.HIGH
            ).all()
            ...
            for high_food in high_fodmap_foods:
                ...
                low_alternatives = self.db.query(Food).filter(
                    and_(
                ...
                if low_alternatives:
                    alternatives[high_food.name] = [alt.name for alt in low_alternatives]
    ...
    def get_personalized_nutrition_guidelines(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "meal_timing": self._get_meal_timing_recommendations(user, recent_symptoms),
                "hydration": self._get_hydration_recommendations(user),
                "supplements": self._get_supplement_recommendations(user, recent_symptoms),
    ...
    def _get_diet_patterns(self, user_id: int, days: int = 30) -> List[DietLog]:
        ...
        cutoff_date = datetime.utcnow() - timedelta(days=days)
        return self.db.query(DietLog).filter(
            and_(
    ...
    def _get_meal_timing_recommendations(self, user: User, symptoms: List[SymptomLog]) -> Dict[str, Any]:
    ...
    def _get_supplement_recommendations(self, user: User, symptoms: List[SymptomLog]) -> List[Dict[str, Any]]:
    ...
    def _calculate_personalization_score(self, user_id: int) -> float:
        ...
        try:
            ...
            symptom_count = self.db.query(SymptomLog).filter(
                SymptomLog.user_id == user_id
            ).count()
            ...
            diet_count = self.db.query(DietLog).filter(
                DietLog.user_id == user_id
            ).count()
    ...
    def _calculate_data_consistency(self, user_id: int) -> float:
        ...
        try:
            ...
            days_with_logs = self.db.query(
                func.date(SymptomLog.logged_at)
            ).filter(
                and_(
                    SymptomLog.user_id == user_id,
                    SymptomLog.logged_at >= cutoff_date
    ...
    def get_dynamic_recommendations(self, user_id: int, risk_level: RiskLevel) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if risk_level == RiskLevel.HIGH:
                recommendations.extend(self._get_high_risk_recommendations(user, recent_symptoms))
            elif risk_level == RiskLevel.MEDIUM:
                recommendations.extend(self._get_medium_risk_recommendations(user, recent_symptoms))
            else:
                recommendations.extend(self._get_low_risk_recommendations(user, recent_symptoms))
    ...
    def _get_high_risk_recommendations(self, user: User, symptoms: List[SymptomLog]) -> List[Dict[str, Any]]:
    ...
    def _get_medium_risk_recommendations(self, user: User, symptoms: List[SymptomLog]) -> List[Dict[str, Any]]:
    ...
    def _get_low_risk_recommendations(self, user: User, symptoms: List[SymptomLog]) -> List[Dict[str, Any]]:
    ...
    def _get_diet_recommendations(self, user: User, diet_patterns: List[DietLog]) -> List[Dict[str, Any]]:
        ...
        if diet_patterns:
            ...
            if food_frequency:
                frequent_foods = sorted(food_frequency.items(), key=lambda x: x[1], reverse=True)[:5]
                food_ids = [food_id for food_id, _ in frequent_foods]
                
                foods = self.db.query(Food).filter(Food.id.in_(food_ids)).all()
                high_fodmap_foods = [f for f in foods if f.fodmap_level == FODMAPLevelEnum.HIGH]
                ...
                if high_fodmap_foods:
                    recommendations.append({
                        ...
                        "actions": [
                            f"Try alternatives to {food.name}" for food in high_fodmap_foods[:3]
                        ],
                        "confidence": 0.6
                    })
    ...
    def _get_fallback_recommendations(self, risk_level: RiskLevel) -> List[Dict[str, Any]]:
