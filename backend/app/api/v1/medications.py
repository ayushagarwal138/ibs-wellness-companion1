...
async def create_medication_log(
    ...
    # Simple implementation that doesn't require full medication reference
    # In production, you'd want a proper Medication table and reference
...
async def get_medication_logs(
    ...
    medication_type: Optional[MedicationTypeEnum] = Query(
        None, description="Filter by medication type"
    ),
    adherence_status: Optional[AdherenceEnum] = Query(
        None, description="Filter by adherence status"
    ),
    medication_name: Optional[str] = Query(
        None, description="Filter by medication name"
    ),
    ...
    offset = (page - 1) * size
    items = (
        query.order_by(desc(MedicationLog.taken_at)).offset(offset).limit(size).all()
    )
...
async def get_medication_stats(
    ...
    taken_count = base_query.filter(
        MedicationLog.adherence == AdherenceEnum.TAKEN
    ).count()
    ...
    status_counts = (
        base_query.with_entities(
            MedicationLog.adherence, func.count(MedicationLog.id).label("count")
        )
        .group_by(MedicationLog.adherence)
        .all()
    )
...
async def get_adherence_report(
    ...
    # Missed doses
    missed_doses = []
    missed_logs = base_query.filter(
        MedicationLog.adherence == AdherenceEnum.MISSED
    ).all()

    for log in missed_logs:
        missed_doses.append(
            {
                "medication_name": "Placeholder Medication",  # Need table join
