...
@router.get("/stats/summary", response_model=SymptomStats)
async def get_symptom_stats(
    ...
    base_query = select(SymptomLog).filter(
        and_(
            SymptomLog.user_id == current_user.id,
    ...
    # Most common symptom (simplified for now)
    most_common_symptom = None
    if logs:
        # Get the first symptom type as placeholder
        most_common_symptom = logs[0].symptom_type.value if hasattr(logs[0], 'symptom_type') else None
    ...
    # Symptoms by severity (simplified)
    symptoms_by_severity = {}
    if logs:
        for log in logs:
            severity_key = log.severity.value if hasattr(log.severity, 'value') else str(log.severity)
            symptoms_by_severity[severity_key] = symptoms_by_severity.get(severity_key, 0) + 1
    ...
    return SymptomStats(
        ...
        average_severity=float(avg_severity) if avg_severity else None,
        symptoms_by_type=symptoms_by_type,
        symptoms_by_severity=symptoms_by_severity,
