    ...
    try:
        ...
        for table_name, stats in storage_stats.items():
            tables_data[table_name] = {
                ...
                "last_analyzed": stats.last_analyzed.isoformat() if stats.last_analyzed else None
            }
        ...
        for table_name, stats in storage_stats.items():
            if stats.total_size_mb > 100:  # Tables larger than 100MB
                recommendations.append(f"Consider archiving old data in {table_name} (current size: {stats.total_size_mb:.1f}MB)")
            
            if stats.row_count > 10000 and stats.avg_row_size_bytes > 1000:  # Large rows
                recommendations.append(f"Consider compressing large fields in {table_name}")
...
async def run_optimization(
    ...
    try:
        ...
        if request.operations:
            ...
            for operation in request.operations:
                if operation == "cleanup":
                    ...
                    result = await optimization_service.optimize_diet_logs(request.level)
                    metrics.append(result)
        ...
        return OptimizationResponse(
            success=True,
            message=f"Optimization completed successfully with {len(metrics)} operations",
...
async def _run_background_optimization(
    optimization_service: DataOptimizationService, 
