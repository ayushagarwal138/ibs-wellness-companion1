...
async def save_onboarding_data(
    ...
    try:
        ...
        if "diagnosisYear" in onboarding_data:
            profile_updates["diagnosis_date"] = datetime(
                int(onboarding_data["diagnosisYear"]), 1, 1
            )
        ...
        # Store allergies in dedicated field (User model doesn't have allergies field)
        # if onboarding_data.get('allergies'):
        #     profile_updates['allergies'] = ', '.join(onboarding_data['allergies'])

        # Store medications in dedicated field (User model lacks field)
        # if onboarding_data.get('medications'):
        #     profile_updates['current_medications'] = ', '.join(
        #         onboarding_data['medications'])
        ...
        updated_user = await user_service.update_user_profile(
            db=db, user_id=current_user.id, profile_data=profile_updates
        )
...
async def get_user_profile(
    ...
    try:
        ...
        return {
            ...
            and onboarding_data.get("onboarding_completed", False),
        }
...
async def update_user_profile(
    ...
    try:
        ...
        updated_user = await UserService.update_user(
            db=db, user_id=current_user.id, user_update=profile_update
        )
...
async def get_onboarding_status(current_user: User = Depends(get_current_active_user)):
    ...
    try:
        ...
        if current_user.medical_notes:
            try:
                onboarding_data = json.loads(current_user.medical_notes)
                onboarding_completed = onboarding_data.get(
                    "onboarding_completed", False
                )
...
async def delete_user_account(
    ...
    - Delete all user data including profile, symptoms, diet logs, etc.
    - Remove all associated ML predictions and analytics
    ...
    try:
        ...
        # Use the UserService static method to delete the user
        # This will cascade delete all associated data due to foreign key constraints
        deletion_success = await UserService.delete_user(db=db, user_id=current_user.id)
        ...
        logger.info(f"Account successfully deleted for user {current_user.id}")
