...
async def stream_predictions(
    ...
    async def generate_predictions():
        ...
        try:
            ...
            # Prepare user data
            user_data = await _prepare_enhanced_user_data(
                current_user, db, request.symptoms
            )
            ...
            if request.include_trends:
                yield _format_stream_response(
                    "flareup_start", {"message": "Computing flareup risk..."}, 0.0, 0
                )
            ...
            yield _format_stream_response(
                "error", {"error": str(e), "message": "Prediction failed"}, 0.0, 0
            )
...
async def enhanced_prediction(
    ...
    try:
        ...
        user_data["temporal_features"] = await _extract_temporal_features(
            current_user.id, db
        )
        ...
        combined_result = {
            ...
            "insights": await _generate_insights(
                severity_result, flareup_result, user_data
            ),
            ...
            for pred_type in request.prediction_types:
                if pred_type == "severity":
                    batch_result["predictions"][
                        "severity"
                    ] = ml_service.predict_severity(user_data)
                    ...
                    batch_result["predictions"][
                        "recommendations"
                    ] = ml_service.generate_recommendations(user_data)
...
async def _get_enhanced_symptom_trends(user_id: str, db: AsyncSession) -> Dict[str, Any]:
    ...
    last_7_days = [
        ...
        if s.logged_at >= datetime.utcnow() - timedelta(days=7)
    ]

    severity_map = {"mild": 1, "moderate": 2, "severe": 3}

    avg_severity_7d = sum(
        severity_map.get(s.severity.value if s.severity else "mild", 1)
        for s in last_7_days
    ) / max(len(last_7_days), 1)
    avg_severity_14d = sum(
        severity_map.get(s.severity.value if s.severity else "mild", 1)
        for s in recent_symptoms
    ) / len(recent_symptoms)
    ...
    return {
        ...
        if avg_severity_7d < avg_severity_14d
        else "worsening"
        if avg_severity_7d > avg_severity_14d
        else "stable",
        "worst_day_severity": max(
            severity_map.get(s.severity.value if s.severity else "mild", 1)
            for s in recent_symptoms
        ),
    }
...
async def _async_severity_prediction(
...
async def _async_flareup_prediction(
    user_data: Dict[str, Any], service: EnhancedRecommendationService
) -> Dict[str, Any]:
