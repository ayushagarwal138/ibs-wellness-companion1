...
async def get_personalization_profile(
    ...
    try:
        ...
        # This ensures the endpoint works while the service is being developed
    ...
    try:
        ...
        for key, value in thresholds.items():
            ...
            if not 0.0 <= value <= 1.0:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Threshold values must be between 0.0 and 1.0",
                )

        # Update thresholds
        updated_thresholds = (
            await personalization_service._update_personalized_thresholds(
...
async def get_adaptive_recommendations(
    ...
    try:
        ...
        user_data = {"user_id": current_user.id, "timeframe": timeframe}
...
async def get_personalization_analytics(
    ...
    try:
        ...
        learning_patterns = (
            await personalization_service._analyze_user_learning_patterns(
                current_user.id
            )
        )
        ...
        effectiveness_metrics = {
            ...
            "recommendation_effectiveness": learning_patterns.get(
