...
from app.core.dependencies import get_current_user import get_current_user
...
async def get_appointments(
    ...
    return AppointmentListResponse(
        appointments=appointments, total=total, skip=skip, limit=limit
    )
...
async def create_appointment(
    ...
    appointment = Appointment(user_id=current_user.id, **appointment_data.dict())
...
async def update_appointment(
    ...
    if "scheduled_date" in update_data or "scheduled_time" in update_data:
        new_date = update_data.get("scheduled_date", appointment.scheduled_date)
        new_time = update_data.get("scheduled_time", appointment.scheduled_time)
...
async def get_upcoming_appointments(
    ...
    appointments = (
        ...
        .order_by(
            Appointment.scheduled_date.asc(), Appointment.scheduled_time.asc()
        )
        .limit(limit)
        .all()
    )
