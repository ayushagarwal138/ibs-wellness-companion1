...
async def login(
    ...
    # Get user data by authenticating again (we already know credentials are valid)
...
async def resend_verification(
    ...
    auth_service = AuthService(db)
    verification_token = await auth_service.create_email_verification(current_user)
...
async def forgot_password(
    ...
    """
    auth_service = AuthService(db)
    reset_token = await auth_service.create_password_reset(password_reset_data.email)
    
    # Always return success to prevent email enumeration
    # TODO: Send password reset email in background if user exists
    # if reset_token:
    #     background_tasks.add_task(send_password_reset_email, password_reset_data.email, reset_token)
