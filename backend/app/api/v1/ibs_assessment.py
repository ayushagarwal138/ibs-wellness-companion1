...
from ...services.ibs_assessment_service import IBSAssessmentService, IBSAssessmentResult
...
class AssessmentRequest(BaseModel):
    ...
    include_recent_data: bool = Field(default=True, description="Include recent symptom/diet data")
    assessment_type: str = Field(default="comprehensive", description="Type of assessment")
    custom_data: Optional[Dict[str, Any]] = Field(default=None, description="Additional assessment data")
...
class QuickAssessmentRequest(BaseModel):
    ...
    bowel_movement_frequency: float = Field(gt=0, description="Daily bowel movement frequency")
...
class RecommendationFilter(BaseModel):
    """Filter model for recommendations."""
    recommendation_types: Optional[List[str]] = Field(default=None, description="Filter by recommendation types")
    priority_levels: Optional[List[str]] = Field(default=None, description="Filter by priority levels")
    implementation_timeframe: Optional[str] = Field(default=None, description="Filter by implementation timeframe")
    
    @validator('recommendation_types')
    def validate_recommendation_types(cls, v):
        if v is not None:
            valid_types = ['dietary', 'lifestyle', 'medical', 'behavioral', 'supplement']
            for rec_type in v:
...
async def conduct_comprehensive_assessment(
    ...
    try:
        ...
        assessment_result = assessment_service.conduct_comprehensive_assessment(
            ...
            db=db,
            assessment_data=request.custom_data
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to conduct IBS assessment: {str(e)}"
        )
...
async def conduct_quick_assessment(
    ...
    try:
        ...
        assessment_result = assessment_service.conduct_comprehensive_assessment(
            user_id=current_user.id,
            db=db,
            assessment_data=quick_assessment_data
        )
        ...
        logger.error(f"Error conducting quick assessment for user {current_user.id}: {e}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to conduct quick IBS assessment: {str(e)}"
        )
...
async def get_personalized_recommendations(
    ...
    include_evidence: bool = Query(default=True, description="Include scientific evidence"),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        implementation_timeline = _create_implementation_timeline(filtered_recommendations)
...
async def get_risk_factors_analysis(
    ...
    try:
        ...
        risk_analysis = {
            ...
            'flare_probability': assessment_result.risk_assessment.get('flare_probability', 0.3),
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to analyze risk factors: {str(e)}"
        )
...
async def get_severity_assessment(
    ...
    try:
        ...
        severity_assessment = {
            ...
            'clinical_severity_score': assessment_result.risk_assessment.get('clinical_severity_score', 0),
            'functional_impact': assessment_result.risk_assessment.get('functional_impact', 'moderate'),
            'psychological_impact': assessment_result.risk_assessment.get('psychological_impact', 'moderate'),
            ...
            'severity_indicators': {
                ...
                'psychological_distress': _extract_psychological_distress(assessment_result),
                'functional_disability': _extract_functional_disability(assessment_result)
            },
        ...
        logger.error(f"Error conducting severity assessment for user {current_user.id}: {e}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to conduct severity assessment: {str(e)}"
        )


@router.get("/assessment-history", response_model=StandardResponse[List[Dict[str, Any]]])
async def get_assessment_history(
    limit: int = Query(default=10, ge=1, le=50, description="Number of assessments to retrieve"),
    ...
    try:
        ...
        assessment_history = [
            {
                ...
                'assessment_date': (datetime.now() - timedelta(days=i*30)).isoformat(),
        ...
        logger.error(f"Error fetching assessment history for user {current_user.id}: {e}")
        raise HTTPException(
            status_code=500,
    ...
    try:
        ...
        update_result = {
            ...
            'new_recommendations': len([
                rec for recs in assessment_result.recommendations.values() 
                for rec in recs
            ]),
            'clinical_flags': assessment_result.clinical_flags,
...
# Helper functions
def _create_assessment_summary(assessment_result: IBSAssessmentResult) -> Dict[str, Any]:
    ...
    return {
        ...
        'confidence_level': 'high' if assessment_result.confidence_score > 0.8 else 'moderate'
    }
...
def _apply_recommendation_filters(
    ...
    for category, recs in recommendations.items():
        ...
        if filters.implementation_timeframe:
            filtered_recs = [
                rec for rec in filtered_recs 
                if _matches_timeframe(rec.get('expected_timeline', ''), filters.implementation_timeframe)
            ]
...
def _generate_risk_insights(assessment_result: IBSAssessmentResult) -> List[str]:
    ...
    if risk_level == 'high':
        insights.append("Your current risk level is elevated and requires immediate attention.")
    
    if 'stress_management_needed' in clinical_flags:
        insights.append("Stress appears to be a significant factor in your IBS symptoms.")
...
def _extract_bowel_dysfunction(assessment_result: IBSAssessmentResult) -> Dict[str, Any]:
...
def _extract_psychological_distress(assessment_result: IBSAssessmentResult) -> Dict[str, Any]:
...
def _extract_functional_disability(assessment_result: IBSAssessmentResult) -> Dict[str, Any]:
...
def _calculate_assessment_changes(
    user_id: str, 
    new_assessment: IBSAssessmentResult, 
    db: Session
) -> Dict[str, Any]:
    """Calculate changes from previous assessment."""
...
async def _schedule_follow_up_tasks(assessment_result: IBSAssessmentResult, db: Session):
