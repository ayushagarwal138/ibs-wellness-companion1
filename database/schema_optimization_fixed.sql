-- Database Schema Optimization Script (Fixed Version)
...
CREATE INDEX IF NOT EXISTS idx_food_items_category_fodmap 
ON food_items (category, fodmap_level);

-- Index for food item name searches (case-insensitive)
CREATE INDEX IF NOT EXISTS idx_food_items_name_lower 
...
CREATE INDEX IF NOT EXISTS idx_ml_predictions_user_created 
...
-- Step 6: Add check constraints for data integrity (using DO blocks to handle IF NOT EXISTS)
DO $$
BEGIN
    RAISE NOTICE 'Step 6: Adding data integrity constraints...';
    
    -- Check if email format constraint exists, if not add it
    IF NOT EXISTS (
    ...
    -- Check if age range constraint exists, if not add it
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
...
CREATE INDEX IF NOT EXISTS idx_user_dashboard_summary_user_id 
